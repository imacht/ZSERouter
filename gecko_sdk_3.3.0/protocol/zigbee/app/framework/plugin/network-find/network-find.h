 ...
 *
 * (NOTE: This component is deprecated as it is not compliant with
 * Zigbee 3.0 requirements. See Network Creator and Network Steering components
 * instead for Zigbee 3.0-compliant implementations of this behavior.)
 * Provides legacy Zigbee Home Automation (pre-3.0) implementation of
 * routines for finding and joining any viable network via scanning,
 * rather than joining a specific network. If you are planning to deploy
 * your application in an environment with potentially lots of
 * open networks, you may wish to implement a mechanism for network
 * blacklisting. Network blacklisting may be accomplished by implementing
 * the emberAfPluginNetworkFindJoinCallback. To extend the functionality to
 * sub-GHz channels, also enable the Network Find (Sub-GHz) plugin.
