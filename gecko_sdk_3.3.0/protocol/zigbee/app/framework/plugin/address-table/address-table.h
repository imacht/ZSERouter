 ...
 * sync with the NCP's address table. When building the Host,
 * ensure that the NCP can support the address
 * table size selected below. The optimal size varies based on the platform
 * architecture and size of other dynamically-allocated data for this application.
 * If errors occur upon reseting the NCP, NCP may not have enough room
 * to store a large address table size.
 * If this is not the case, the NCP may not be able to dynamically increase
 * the address table size based on the Host's request. You can work around
 * this issue by statically allocating the desired address table size on
 * the Zigbee-pro-stack/leaf library components on your NCP.
 *
 */
