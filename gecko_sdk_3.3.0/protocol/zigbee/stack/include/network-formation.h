...
/** @brief Forms a new Wirefree network.
 *
 * @note If using security, the application must call
 *   ::emberSetInitialSecurityState() prior to joining the network.  This also
 *   applies when a device leaves a network and wants to form another one.
...
/** @brief Function callback that is used to handle the reception of a beacon.
 *
 * NOTE: SoC only
 */
...
/** @brief Function callback that is used to handle the conclusion of an active
 * or energy scan.
 *
 * NOTE: SoC only
 */
...
/** @brief Kicks off a procedure to scan for beacons, filter results, and
...
/*
 * @brief could be used to extend a joiner's timeout to wait for the network key
 ...
 * We can achieve the same EZSP applicable setting with emberSetBeaconClassificationParams
 */
 ...
 * This is left here for backward compatibility.
 * We can achieve the same with emberSetBeaconClassificationParams
 *
 */
 ...
 * This is left here for backward compatibility.
 * We can achieve the same with emberGetBeaconClassificationParams
 *
 */
...
/*
 * @brief This is to get the last LQI/RSSI values for neighbours in a specific debug mode
 *
 *  @param nodeId id of the node for which we are looking for lqi and rssi
 *
 *  @returns EMBER_SUCESS if the specific defines exist, otherwise returs EMBER_FATAL_ERR
...
/*
 * @brief This sends an update PAN ID message, with the new PanID, it could be called
...
/** @brief Clears all cached beacons that have been collected from an active
   scan. */
void emberClearStoredBeacons(void);

/*
 * @brief Configures the number of beacons to store when issuing active scans
...
/*
 * @brief Returns the first beacon in the cache. Beacons are stored in cache
...
/*
 * @brief Returns the next beacon in the cache. Beacons are stored in cache
 *  after issuing an active scan.
 *
...
/** @brief Returns the number of cached beacons that have been collected from a
   scan. */
