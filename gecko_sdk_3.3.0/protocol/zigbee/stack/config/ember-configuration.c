...
#define END_emNeighborData                                                               \
  (END_emPanInfoData + align(((EMBER_NEIGHBOR_TABLE_SIZE * EMBER_ZC_AND_ZR_DEVICE_COUNT) \
...
#define END_emFrameCountersTable                                                                                   \
  (END_emNeighborData + align(((EMBER_NEIGHBOR_TABLE_SIZE + EMBER_CHILD_TABLE_SIZE) * EMBER_ZC_AND_ZR_DEVICE_COUNT \
...
#if defined (CORTEXM3)
// EMZIGBEE-6105: clean up the following code when
// "Zigbee on UC - Host/NCP migration" is done.
#ifdef UC_BUILD
// For UC apps, the EMHEAP segment no longer exists
uint8_t emAvailableMemory[END_stackMemory] __ALIGNED(4);;
#else
// For none-UC apps in particular of none-UC Host-NCP,
// as the memory-allocation.c has not yet been rearchitected
// due to the elimination of EMHEAP, we still need this one to
// make none-UC Host-NCP work properly.
...
#endif
uint8_t emAvailableMemory[END_stackMemory + SIMULATED_EXTRA_MEMORY] __attribute__ ((aligned(4)));
