...
emberAfPluginFormAndJoinMarker();
}
...
emAfPluginFormAndJoinEnergyScanResultCallback(channel, maxRssiValue);
emAfPluginScanDispatchEnergyScanResultCallback(channel, maxRssiValue);
}
...
emAfPluginFormAndJoinNetworkFoundCallback(networkFound, lqi, rssi);
emAfPluginScanDispatchNetworkFoundCallback(networkFound, lqi, rssi);
}
...
emAfPluginFormAndJoinScanCompleteCallback(channel, status);
emAfPluginScanDispatchScanCompleteCallback(channel, status);
}
...
emAfPluginFormAndJoinUnusedPanIdFoundCallback(panId, channel);
...
emberAfPluginPartnerLinkKeyExchangeZigbeeKeyEstablishmentCallback(partner, status);
}
...
emAfPluginCountersCounterCallback(type, Info);
}
